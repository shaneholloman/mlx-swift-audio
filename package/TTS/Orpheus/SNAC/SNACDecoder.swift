...
class SNACDecoder {
  ...
  init(config: SNACConfig, weights: [String: MLXArray]) {
    ...
    initialConv = { (x: MLXArray) -> MLXArray in
    ...
    decoderBlocks = blocks

    // 3. Final Layers
    let finalBlockIndex = 6 // for decoder.model.layers.6.alpha
    finalSnakeAlpha = weights["decoder.model.layers.\(finalBlockIndex).alpha"]!
    ...
    finalConv.weightG = finalConvWeightG
    finalConv.weightV = finalConvWeightV
    finalConv.bias = finalConvBias
...
//        let normalizedArray = floatArray.map { $0 * normFactor }
  ...
  private func embedCodes(codes: [[Int]]) -> MLXArray {
    ...
    if max_expanded_length == 0, !codes.isEmpty {
      Log.tts.warning("max_expanded_length is 0, but codes exist. This might happen if all code chunks are empty.")
    } else if max_expanded_length == 0 {
      Log.tts.warning("max_expanded_length is 0 (no codes). Decoder might not produce output.")
    }
    ...
    for i in 0 ..< config.vqStrides.count {
      ...
      guard let outProjWeightG = weights["quantizer.quantizers.\(i).out_proj.weight_g"],
            ...
            let outProjBias = weights["quantizer.quantizers.\(i).out_proj.bias"]
      else {
      ...
      if expanded_z_q_i.shape == z_q_sum.shape {
        ...
        Log.tts.warning("Shape mismatch for VQ \(i). Expected \(z_q_sum.shape), got \(expanded_z_q_i.shape)")
      }
    }
